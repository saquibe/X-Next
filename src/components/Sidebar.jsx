...
export default function Sidebar() {
  ...
  return (
    <div className="flex flex-col p-3 justify-between h-screen">
      ...
      {session ? (
        <button className="bg-blue-400 text-white rounded-full hover:brightness-95 transition-all duration-200 w-48 h-9 shadow-md font-semibold hidden lg:inline"
        onClick={() => signOut()}
        >
          Sign Out
        </button>
  ...
  >
      ...
      {session && (
        <div className="text-gray-700 text-sm flex items-center cursor-pointer p-3 hover:bg-gray-100 rounded-full transition-all duration-200">
            <img src={session.user.image} alt="user-img" className="h-10 w-10 rounded-full lg:mr-2" />
